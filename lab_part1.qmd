...
The histogram tells us *how much* damage there is overall, but not *where*. Plotting severe-damage percentage against distance from the blast epicentre gives us the spatial structure in a single chart — and a sanity check on the map.
...
damage_zones |>
  ...
  labs(
    ...
    subtitle = "Each point is one zone · LOESS smoother in navy",
...
Rather than a plain table, we rank the top 10 cadastres/zones by its distance from the blast epicentre. This pulls together *magnitude* and *geography*.
